...
def main():
    ...
    max_force = math.ceil(df_all[2].max())

    df_training_frames = [df_training]
    df_training = pd.concat(df_training_frames)
    df_training.columns = [type_name, 'horizontal_displacement', 'training']

    df_target_frames = [df_target]
    df_target = pd.concat(df_target_frames)
    df_target.columns = [type_name, 'horizontal_displacement', 'target']

    #df_training_frames = [df_training_1]
    #df_training = pd.concat(df_training_frames)
    #df_training.columns = ['pert', 'horizontal_displacement', 'target', 'training', 'test_error']

    df_test_frames = [df_test]
    df_test = pd.concat(df_test_frames)
    df_test.columns = [type_name, 'horizontal_displacement', 'target', 'test', 'test_error']
    ...
    plt.ylabel('Force (kN)')
    training_plt = plt.scatter(df_training['horizontal_displacement'], df_training['training'], color='lightcoral', s=4, alpha=1)
    ...
    legend = plt.legend(handles=[training_plt, target_plt, test_plt], loc=2, framealpha=0.5)
